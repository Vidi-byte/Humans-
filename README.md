# Installing the Humans Node

Humans - is a sovereign next-generation blockchain platform that brings together an ecosystem of stakeholders around the use of AI to create large-scale projects.

The token is already being traded, but the project decided to move from Polkadot to Cosmos. Now the testnet is running without rewards, but the next stage will be awardable, and then exit to mainnet.

As far as requirements go, this is a typical Cosmosfork, so I ran a validator just in case. Since the token is already traded, I don't expect a lot of rewards from this project, but there is room on the server, let it spin.
...
### Project website (take a survey on the site by entering your email)
